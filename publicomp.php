...
$query1 = "select * from album, band where album.Band_name=band.Band_name and band.Company_name='".$compname."'";
$resAA = mysqli_query($con, $query1);
$nAA = mysqli_num_rows($resAA);
...
while($row=mysqli_fetch_assoc($resAA)){
  $x = $row['Album_name'];
  $y = $row['album_cover'];
  $AresArray[] = $x;
  $ApicArray[] = $y;
